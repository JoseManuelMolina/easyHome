                      ...
                      <tr>
                        <th scope="row">
                          <div class="media align-items-center">
                            <div class="media-body">
                              <span class="name mb-0 text-sm">{{ $usuario -> name }}</span>
                            </div>
                          </div>
                        </th>
                        <td class="budget">
                        ...
                        <td>
                          <span class="badge badge-dot mr-4">
                            <span class="status">{{ $usuario -> email }}</span>
                          </span>
                        </td>
                        <td>
                          <span class="status">{{ $usuario -> rol }}</span>
                        </td>
                        <td class="text-right">
                          <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                <a class="dropdown-item" href="{{ url('admin/useredit/' . $usuario->id) }}">Editar Usuario</a>
                                <form method="post" action="{{ url('user/destroy/' . $usuario->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <div class="card-footer" style="text-align:center">
                                    <button type="submit" onclick="return confirm('¿Estas seguro de que quieres eliminar este inmueble?')">Eliminar usuario</button>
                                </div>
                                </form>
                            </div>
                          </div>
